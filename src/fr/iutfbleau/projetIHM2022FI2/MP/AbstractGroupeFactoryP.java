...
public class AbstractGroupeFactoryP implements AbstractGroupeFactory {
        ...
        try {
            ...
            try {
                ...
                if(!res.isBeforeFirst()) {
                    System.err.println("Il n'y a pas de ROOT");
                } else {
                    res.next();
    ...
    private void loadGroupes() {
        try {
            ...
            try {
                ...
                ResultSet res = req.executeQuery();
                while(res.next()) {
                    Groupe g = new GroupeP(res.getInt(1), res.getString(2), res.getInt(3), res.getInt(4), res.getString(5), this.brain.get(res.getInt(6)));
        ...
        try {
            ...
            try {
                ...
                if (generatedKeys.next()) {
                    Groupe g = new GroupeP(generatedKeys.getInt(1),name,min,max,"FREE",pere);
    ...
    public Boolean knows(Groupe g){
        try {
            ...
            Connection conn = this.pool.getConnection();
            try {
                PreparedStatement req = conn.prepareStatement("SELECT * FROM Groupe WHERE id = ?");
                //------ Requete preparee -------- 
    ...
    public void createPartition(Groupe pere, String name, int n){
        ...
        try {
            ...
            try {
                ...
                if (generatedKeys.next()) {
                    req.close();
                    Groupe copiePereRacinePartition = new GroupeP(generatedKeys.getInt(1),nameG + "_" + numero,min,max,"PARTITION",pere);
    ...
    private void addSousGroupePartition(Groupe g, Etudiant student) {
        ...
        for(Groupe sousGroupe : groupes) {
            if(sousGroupe.getType().equals(TypeGroupe.PARTITION)) {
                addSousGroupePartition(sousGroupe, student);
            }
            else{
                if(!found && sousGroupe.getSize() < min) {
    ...
    public void dropFromGroupe(Groupe g, Etudiant e){
        ...
        if(g.getType() == TypeGroupe.PARTITION){ // Si c'est une partition 
            for(Groupe groupe : g.getPointPoint().getSousGroupes() ){ // Suppression dans ses frères 
                if(!g.equals(groupe) && groupe.getEtudiants().contains(e) )
                    dropFromGroupe(groupe, e);
            }
        }
    }
